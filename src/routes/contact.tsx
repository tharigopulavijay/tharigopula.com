  ...
  head: () => ({
    meta: [
      { title: "Contact — Talk To A Technology Partner | Tharigopula Technologies" },
      {
        name: "description",
        content:
          "Tell us about your business and what is slowing it down. We reply with a practical direction, not a sales pitch.",
      },
      { property: "og:title", content: "Contact Tharigopula Technologies" },
      {
        property: "og:description",
        content: "Start a conversation about your website, software or automation project.",
      },
    ],
  }),
...
function ContactPage() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Records that an enquiry converted and whether it carried estimator scope.
      // No name, email, phone or message content is ever sent.
      track({
        name: "contact_submitted",
        source: handoff ? "estimator" : "contact",
        hadEstimate: Boolean(handoff),
      });
      ...
      toast.error(
        "That did not send. Please use WhatsApp or email below — we do not want to lose your message.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const field =
    "mt-1.5 w-full rounded-md border border-border bg-card px-3.5 py-2.5 text-sm outline-none focus:border-foreground/40";
          ...
          {sent ? (
            <div className="rounded-xl border border-signal bg-signal/5 p-6 sm:p-8">
              ...
              <h2 className="mt-3 font-display text-2xl font-semibold">
                Thanks, {sent.payload.name}.
              </h2>
              <p className="mt-2 text-sm text-muted-foreground">
                Your reference is{" "}
                <span className="font-mono font-semibold text-foreground">{sent.reference}</span>.
                We reply within one business day.
              </p>
              <p className="mt-5 text-sm">
                Want a faster answer? Send the same details on WhatsApp — it arrives instantly with
              ...
              {handoff ? (
                <div className="mb-6 rounded-lg border border-signal/40 bg-signal/5 p-4">
                  ...
                  <p className="mt-1.5 text-sm text-muted-foreground">
                    We will receive your configurator breakdown with this message, so you will not
                    be asked to explain the scope again.
                  </p>
                </div>
              ) : null}
          ...
          <div className="space-y-5">
            <div className="rounded-xl border border-border bg-card p-6">
              ...
              <a
                ...
                onClick={() => track({ name: "email_clicked", source: "contact" })}
              ...
              <a
                ...
                onClick={() => track({ name: "phone_clicked", source: "contact" })}
              ...
              <a
                href={whatsappLink()}
                onClick={() => track({ name: "whatsapp_clicked", source: "contact" })}
                target="_blank"
                rel="noopener noreferrer"
            ...
            <div className="rounded-xl border border-border bg-secondary/50 p-6 text-sm text-muted-foreground">
              Prefer structure? Use the project configurator to answer a few guided questions and
              receive an indicative estimate along with your enquiry.
            </div>
          </div>
        </div>
      </Section>
