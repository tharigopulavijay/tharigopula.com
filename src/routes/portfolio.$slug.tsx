    ...
    <>
      <section className="surface-ink">
        <Container className="py-14 sm:py-20">
          <SectionHeading ink as="h1" eyebrow={`${study.client} · ${study.industry}`} title={study.title} lead={study.summary} />
            ...
            <div className="mt-4 grid gap-px overflow-hidden rounded-xl border border-border bg-border">
              {study.architecture.map((a) => (
                <div key={a.layer} className="bg-card p-4">
                  <p className="font-mono text-[10px] tracking-[0.16em] text-muted-foreground uppercase">{a.layer}</p>
      ...
      <Section className="bg-secondary/40">
        ...
        <div className="mt-10 flex flex-col gap-3 sm:flex-row">
          ...
          <Link to="/portfolio" className="rounded-md border border-border px-5 py-3.5 text-center text-sm font-semibold">
            Back to portfolio
          </Link>
        </div>
      </Section>
