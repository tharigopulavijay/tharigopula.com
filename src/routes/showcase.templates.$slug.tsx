...
export const Route = createFileRoute("/showcase/templates/$slug")({
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => {
    if (!loaderData)
      return { meta: [{ title: "Template not found" }, { name: "robots", content: "noindex" }] };
  ...
  return (
    <>
      <section className="surface-ink">
        <Container className="py-14 sm:py-20">
          ...
          <p className="mt-3 text-sm text-ink-muted">
            A representation of this direction. Below, the same experience level is running live —
            click inside it, resize it, or open it full screen.
          </p>
        </Container>
      </section>
      ...
      <Section className="bg-secondary/40">
        ...
        <div className="mt-10 rounded-xl border border-border bg-secondary/50 p-6">
          ...
          <p className="mt-3 max-w-2xl text-sm text-muted-foreground">
            Start with a direction you like. We customise the design, content, workflows and
            functionality around your business — the final scope decides the final price.
          </p>
      ...
      {siblings.length ? (
        <Section className="bg-secondary/40">
          <SectionHeading
            eyebrow="Similar directions"
            title={`More ${category?.name.toLowerCase()} styles`}
          />
